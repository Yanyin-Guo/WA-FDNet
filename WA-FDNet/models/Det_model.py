...
class Det_YOLO(BaseModel):
    def __init__(self, opt):
        super(Det_YOLO, self).__init__(opt)
    ...
    def init_training_settings(self):
        ...
        self.loss_FD = build_loss(self.opt['train']['Loss_det']).to(self.device)
        self.balancer = FAMO(n_tasks=2, device="cuda", gamma=0.001)

    def setup_optimizers(self):
        train_opt = self.opt['train']
        self.optimizer_g_netencoder = self.build_optimizer(self.net_encoder, 
                                                           name=train_opt['optimizer_encoder']['type'], 
                                                           lr=train_opt['optimizer_encoder']['lr'],
                                                           momentum=train_opt['optimizer_encoder'].get('momentum', 0.9),
                                                           decay=train_opt['optimizer_encoder'].get('weight_decay', 1e-5),
                                                           iterations=train_opt['total_iter'])
        self.optimizers.append(self.optimizer_g_netencoder)
        self.optimizer_g_netdetection = self.build_optimizer(self.net_detection, 
                                                           name=train_opt['optimizer_detection']['type'], 
                                                           lr=train_opt['optimizer_detection']['lr'],
                                                           momentum=train_opt['optimizer_detection'].get('momentum', 0.9),
                                                           decay=train_opt['optimizer_detection'].get('weight_decay', 1e-5),
                                                           iterations=train_opt['total_iter'])
    ...
    def _optimize_parameters(self, current_iter):
        ...
        x4_vi,  x5_vi,  x6_vi=self.net_encoder(self.data)
        self.preds = self.net_detection(x4_vi,  x5_vi,  x6_vi)
        loss_det = self.loss_FD(preds=self.preds, batch =self.batch)
    ...
    def nondist_validation(self, dataloader, current_iter, tb_logger, save_img):
        ...
        for idx, val_data in enumerate(dataloader):
            ...
            n=len(visuals['gt_vi'])
            for i in range(n):
                sr_img = tensor2img(visuals['gt_vi'][i].detach(), min_max=(0, 1))
    ...
    def save(self, epoch, current_iter):
        self.save_network([self.net_encoder, self.net_detection], 'net_fe_g', current_iter, param_key=['params_encoder','params_detection'])
        self.save_training_state(epoch, current_iter)
    
    def save_best(self, current_iter):
        logger = get_root_logger()
        self.save_network([self.net_encoder, self.net_detection], 'net_best', current_iter, param_key=['params_encoder','params_detection'])
